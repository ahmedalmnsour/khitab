...
1. **عبارات جندرية** (780 عبارة): تحمل `male` و`female`، وكثيرٌ منها يحمل أيضاً صيغة `neutral` مصدرية حين يكون لفعل الأمر مصدرٌ محايد طبيعي (مثل `view`: `اعرض`/`اعرضي`/`عرض`). أمّا الجُمل الخبرية والماضية المخاطِبة فلا تحمل `neutral`، لأنه لا يكون لها مصدرٌ محايد صادق.
...
لا تُنشئ Factory في النطاق العام (global scope) على الخادم. الدالة الناتجة من `createKhitab` تحمل حالةً (الجنس الافتراضي والإعدادات). إن وُضعت في مستوى الوحدة (module-level) على سيرفر، يؤدي ذلك إلى **مشاركة حالةٍ واحدة** بين كل الطلبات، فيُخاطَب الجميع بإعدادٍ لا يخصّهم؛ ولو اشتُقّ هذا الإعداد من طلبٍ سابق صار **تسرّباً** فعلياً بين الطلبات.
